...
import { assertLoadAccess } from '../lib/ownership';
...
const loadIdParam = z.object({ id: z.string().cuid() });
...
router.get('/:id', validateParams(loadIdParam), asyncHandler(async (req, res) => {
  const params = req.params as z.infer<typeof loadIdParam>;
  const load = await assertLoadAccess(req.user!, params.id);
  return res.json({ item: load });
}));
