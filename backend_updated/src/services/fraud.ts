...
async function createSignal(input: {
  ...
  return prisma.fraudSignal.create({ data: input });
}

export async function evaluateTripLocationRisk(input: {
  tripId: string;
  driverUserId: string;
  lat: number;
  lng: number;
  ...
  const signals: Array<{ signalType: string; severity: Severity; score: number; reason: string }> = [];
  ...
  const previous = await prisma.tripLocationPing.findFirst({
...
export async function evaluatePaymentRisk(input: { userId: string; amountInPaise: number; loadId?: string }) {
  const signals: Array<{ signalType: string; severity: Severity; score: number; reason: string }> = [];
  ...
  const recentFailed = await prisma.paymentOrder.count({
